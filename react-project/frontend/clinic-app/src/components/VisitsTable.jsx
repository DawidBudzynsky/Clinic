...
export default function VisitsTable({ visits, onApply }) {
  ...
  const handleButtonClicked = async (visit_id) => {
    ...
    setRequestData((prevData) => ({ visit_id: visit_id, user_id: response.data.id }));
    ...
    <>
      <div className="px-3 py-3 shadow rounded-3 mb-5" style={{ minHeight: '60vh', backgroundColor: 'white' }}>
        ...
        <table className="table table-striped" >
          ...
          <tbody>
            {visits.filter((item) => ((specialitySearch === '' ? item : item.doctor.speciality.includes(specialitySearch)) &&
              !item.is_reserved
            )).map((visit, index) => (
              <tr key={visit.id} >
